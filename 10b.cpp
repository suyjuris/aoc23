...
int main() {
    ...
    for (s64 i = 0; i < arr.size; ++i) {
        if (arr[i] == 'S') {
            ...
            start = {x, y, 0, 0};
            for (Pos p: stack) {
                hashmap_set(&pred, p.key(), start);
            }
        }
    }
    
    auto mark_path = [&](Pos p, u8* into) {
        u8 last = 4;
        while (true) {
    ...
    while (stack.size) {
        ...
        if (bitset_get(visited, p.x+p.y*width)) {
            ...
            assert(stack.size == 1);
            mark_path(p_orig, &startv);
            mark_path(stack[0], &startv);
            break;
        }
        bitset_set(&visited, p.x+p.y*width, true);
        hashmap_set(&pred, p.key(), p_orig);
    ...
    u8 startc;
    switch (startv) {
        case 0b1010: startc = '-'; break;
        case 0b0101: startc = '|'; break;
        case 0b1100: startc = 'L'; break;
        case 0b0110: startc = 'J'; break;
        case 0b0011: startc = '7'; break;
        case 0b1001: startc = 'F'; break;
        default: assert(false);
    }
    
    arr[start.x + start.y*width] = startc;
    
    s64 count = 0;
    for (s64 y = 0; y < height; ++y) {
        bool inside = false;
        for (s64 x = 0; x+1 < width; ++x) {
            bool vis = bitset_get(visited, x + y*width);
            u8 c = get(x, y);
            if (vis) {
                inside ^= c == '|' or c == '7' or c == 'F';
            } else {
                count += inside;
            }
        }
